...
# with open('../data_source/taipei_newTaipei_districts.json', 'r') as f:
...
def draw_BubbleMap(selectedData, clickData, clickData2, clickData3):
    # DataOfBM = DataOfBM_df.copy()
    # if selectedData is None and clickData is None and clickData2 is None: #Default
    if selectedData is None and clickData is None and clickData2 is None and clickData3 is None: # Default
        df_subset = df.copy()
        df_subset['RoadTotal'] = df_subset['BIGVOLUME'] + df_subset['CARVOLUME'] + df_subset['MOTORVOLUME']
    elif selectedData is not None: # Line Chart
    ...
    elif clickData3 is not None: # Choropleth Map出現範圍外的道路
        label = clickData3['points'][0]['location']
        print(label)
        df_subset = df.copy()
        ...
        df_subset = df_subset[df_subset['Town'] == label]
        # if '西園路二段' in df_subset['RoadName']: # False
        #     print('True')
        # print(df_subset['RoadName']== '西園路二段') 
        print(df_subset['Town'].head(5))
        df_subset['RoadTotal'] = df_subset['BIGVOLUME'] + df_subset['CARVOLUME'] + df_subset['MOTORVOLUME']

    if clickData is not None or selectedData is not None: # Pie Chart or Line Chart
    # if clickData is not None or selectedData is not None or clickData3 is not None: # Pie Chart or Line Chart or Choropleth Map
    ...
    if clickData3 is not None: # Choropleth Map出現範圍外的道路
        grouped = df_subset.groupby(['RoadName', 'Date', 'PositionLon', 'PositionLat'])['RoadTotal'].sum().reset_index()
        if '西園路二段' in grouped['RoadName']:
            print('True')
        pivot_table = grouped.pivot_table(values='RoadTotal', index=['RoadName', 'PositionLon', 'PositionLat'], columns='Date', fill_value=0)
        ...
        # if '西園路二段' in DataOfBM['RoadName']: # True
        #     print('西園路二段True')
...
def draw_ChoroplethMap(selectedData, clickData, selectedData2, clickData2):
    ...
    if selectedData is None and clickData is None and selectedData2 is None and clickData2 is None: # Default
        ...
        df_subset['RoadTotal'] = df_subset['BIGVOLUME'] + df_subset['CARVOLUME'] + df_subset['MOTORVOLUME']
    elif selectedData is not None: # Line Chart不行
        first = selectedData['range']['x'][0]
        last = selectedData['range']['x'][1]
        first = datetime.strptime(first, '%Y-%m-%d %H:%M:%S.%f')
        last = datetime.strptime(last, '%Y-%m-%d %H:%M:%S.%f')
...
def draw_PieChart(selectedData, selectedData2, clickData, clickData2):
    ...
    elif selectedData2 is not None: # Line Chart
        first = selectedData2['range']['x'][0]
        last = selectedData2['range']['x'][1]
        first = datetime.strptime(first, '%Y-%m-%d %H:%M:%S.%f')
        last = datetime.strptime(last, '%Y-%m-%d %H:%M:%S.%f')
    ...
    # elif selectedData is None and selectedData2 is None: #只有Bar Chart點擊
        ...
        TargetRoadName = [get_roadname(point['PositionLon'], point['PositionLat']) for _, point in df_subset.iterrows()]
        df_subset = df[df['RoadName'].isin(TargetRoadName)].copy()
    elif clickData2 is not None: # Choropleth Map
        label = clickData2['points'][0]['location']
        df_subset = df.copy()
        df_subset = df_subset.head(50000)
        geo_dict = {feature['properties']['TOWN']: Polygon(feature['geometry']['coordinates'][0]) 
                for feature in geojson_data['features']}
...
@app.callback( Output('Heap Map', 'figure'),
            ...
            Input('Bar Chart', 'clickData'),
            Input('Choropleth Map', 'clickData') )
def draw_HeapMap(selectedData, selectedData2, clickData, clickData2, clickData3):
    DataOfHM = pd.DataFrame(index=[], columns=[])

    if selectedData is None and selectedData2 is None and clickData is None and clickData2 is None: #Default
    # if selectedData is None and selectedData2 is None and clickData is None and clickData2 is None and clickData3 is None: # Default
        df_subset = df.copy()
    ...
    # elif selectedData is None and selectedData2 is None and clickData is None: # 只有Bar Chart點擊
        ...
        df_subset = df[df['RoadName'].isin(TargetRoadName)].copy()
        df_subset['Total Volume'] = df_subset['BIGVOLUME'] + df_subset['CARVOLUME'] + df_subset['MOTORVOLUME']
    # elif clickData3 is not None: # Choropleth Map造成Heat Map資料短缺
    #     label = clickData3['points'][0]['location']
    #     df_subset = df.copy()
    #     df_subset = df_subset.head(20000)
    #     geo_dict = {feature['properties']['TOWN']: Polygon(feature['geometry']['coordinates'][0])
    #                 for feature in geojson_data['features']}
    #     df_subset['Town'] = [town_id for town_id, polygon in geo_dict.items() for lon, lat in zip(df_subset['PositionLon'], df_subset['PositionLat'])
    #                     if polygon.contains(Point(lon, lat))]
...
@app.callback( Output('Line Chart', 'figure'),
            ...
            Input('Bar Chart', 'clickData'),
            Input('Choropleth Map', 'clickData') )
def draw_LineChart(selectedData, clickData, clickData2, clickData3):
    ...
    # elif selectedData is None and clickData is None: # 只有Bar Chart點擊
        ...
        TargetRoadName = [get_roadname(point['PositionLon'], point['PositionLat']) for _, point in df_subset.iterrows()]
        df_subset = df[df['RoadName'].isin(TargetRoadName)].copy()
    # elif clickData3 is not None: # Choropleth Map造成Line Chart資料短缺
    #     label = clickData3['points'][0]['location']
    #     df_subset = df.copy()
    #     df_subset = df_subset.head(20000)
    #     geo_dict = {feature['properties']['TOWN']: Polygon(feature['geometry']['coordinates'][0])
