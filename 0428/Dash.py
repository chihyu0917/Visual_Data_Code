...
with open('../data_source/taipei_districts.json', 'r') as f:
...
@app.callback( Output('Choropleth Map', 'figure'),
               ...
               Input('Pie Chart', 'clickData'))
            #    Input('Bar Chart', 'clickData') )
# def draw_ChoroplethMap(selectedData, clickData, clickData2):
def draw_ChoroplethMap(selectedData, clickData):
    ...
    #     label = clickData2['points'][0]['label']
    #     DataOfBM = DataOfBM_df[DataOfBM_df['Color'] == label]
    # df_subset['TownID'] = 0 * len(df_subset)
    # for i in range(len(df_subset)):
    #     for j in range(12):
    #         point = Point(df_subset['PositionLat'][i], df_subset['PositionLon'][i])
    #         polygon = Polygon(geojson_data['features'][j]['geometry']['coordinates'][0])
    #         if polygon.contains(point):
    #             df_subset['TownID'][i] = geojson_data['features'][j]['properties']['TOWNID']
    #             break
